    ...
    <Link to={`/shop/fruits/${id}`}>
      <div className='bg-white border border-[#e2e2e2] p-4 md:p-6 rounded-md group hover:shadow-lg transition-all duration-300'>
       ...
       <div className='flex flex-col gap-3 justify-center items-center'>
         <div>
          <img src={image} alt='cucumber' className='h-24 md:h-40 w-auto'/>
        </div>
        <div className='flex flex-col gap-1 justify-center items-center'>
          <p className='lato-bold text-[12px] md:text-[14px] text-[#075212]'>{name}</p>
          <p className='lato-bold text-[12px] md:text-[14px] text-[#075212]'>{desc}</p>
          <div className='flex gap-2'>
          <p className='text-[#FE9C00] lato-regular text-[12px] md:text-[14px]'>${price}</p>
          <p className='lato-regular text-[12px] md:text-[14px] text-[#075212] line-through'>${subprice}</p>
          </div>
          <AddToCart />
        </div>
       </div>
